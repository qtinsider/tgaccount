...
Page {
    ...
    Flickable {
        ...
        Column {
            ...
            Title {
                ...
                subtitle: qsTr("Unofficial Telegram client for Nokia N9")
                width: parent.width
            }

            Label {
                id: country_name
                text: countriesModel.get(selectionDialog.selectedIndex).name
                width: parent.width
                font.pixelSize: 48
            ...
            Column {
                ...
                Row {
                    ...
                    TumblerButton {
                        id: country_code
                        width: 160
                        text: countriesModel.get(selectionDialog.selectedIndex).code
                        onClicked: selectionDialog.open()
                    }
                    TextField {
                        id: phone_number
                ...
                Label {
                    font.pixelSize: 24
                    text: qsTr("Please confirm your country code and\nenter your phone number.")
                }
    ...
    onVisibleChanged: {
        if (visible) {
            phone_number.focus = true;
        }
    }

    function setPhoneNumber() {
        if (phone_number.text.length > 0) {
            actor.setPhoneNumber(country_code.text + phone_number.text)
            pageStack.busy = true
        } else {
            showBanner("Phone number field can't be empty")
        }
    }
    ...
    Component.onCompleted: {
        for (var i = 0; i < countriesModel.count; i++) {
            if (actor.getDefaultIndex(countriesModel.get(i).iso2)) {
